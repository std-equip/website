...
//ВНЕСЕНЫ В БИБЛИОТЕКУ ИЗМЕНЕНИЯ, КАСАЮЩИЕСЯ ВЫСОТЫ.
...
(function( $ ){
	$.fn.printPage = function(options) {
		...
		function loadPrintDocument(el, pluginOptions)
		{
			...
			$('#printMessageBox').css("opacity", 0).animate({opacity:1}, 300, function() { addIframeToPage(el, pluginOptions); });
		}
		...
		{
			let url = (pluginOptions.url) ? pluginOptions.url : $(el).attr(pluginOptions.attr);
			url += '&javascript=1';
